...
export function detectOrphanTmuxSessions() {
    const findings = [];
...
export function detectStaleStates(cwd) {
    const findings = [];
    ...
    for (const { file, label } of stateFiles) {
        const state = readDataFile(cwd, file, null);
        if (!state?.active)
            continue;
...
export function detectStaleWorktrees(cwd) {
    const findings = [];
...
export function runFullDetection(cwd) {
    return [
...
export function formatShutdownReport(findings) {
    if (findings.length === 0)
        return "Shutdown check: clean. No orphan processes or stale state detected.";
    const lines = [
