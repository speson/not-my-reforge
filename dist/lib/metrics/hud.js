...
export function buildHudState(metrics, activeMode) {
    const lastToolEntry = Object.values(metrics.toolCalls).sort((a, b) => new Date(b.lastUsedAt).getTime() - new Date(a.lastUsedAt).getTime())[0];
 ...
 */
export function formatHudForTmux(state) {
    const parts = [
 ...
 */
export function formatHudInline(state) {
    return [
