...
function formatDiscordPayload(msg) {
    return {
        embeds: [
            {
                title: msg.title,
...
function formatSlackPayload(msg) {
    const emoji = msg.event.includes("failed") ? ":x:" : ":white_check_mark:";
    const fields = [];
    if (msg.project)
        fields.push({ type: "mrkdwn", text: `*Project:* ${msg.project}` });
    if (msg.branch)
        fields.push({ type: "mrkdwn", text: `*Branch:* ${msg.branch}` });
    return {
        blocks: [
            {
                type: "header",
...
function formatTelegramPayload(msg, chatId) {
...
function formatGenericPayload(msg) {
    return {
        event: msg.event,
        title: msg.title,
        body: msg.body,
...
export async function sendNotification(channel, msg) {
    let payload;
...
export async function broadcastNotification(channels, msg) {
    const enabled = channels.filter((c) => c.enabled);
    const results = await Promise.allSettled(enabled.map((c) => sendNotification(c, msg)));
    let sent = 0;
    let failed = 0;
