...
export function analyzeTask(task) {
    ...
    return {
        ...
        isIterativeFix: /\b(fix\s+(all|every|the|각|모든)|until\b|반복|--verify)/i.test(lower),
        isTestRelated: /\b(test|테스트|failing|fail|pass|coverage|jest|pytest|vitest|mocha|cargo\s+test)/i.test(lower),
        isResearch: /\b(analy[zs]e|분석|investigate|research|audit|survey|explore|compare)/i.test(lower),
        isPlanning: /\b(plan|설계|design|architect|schema|strategy|propose|roadmap|redesign)/i.test(lower),
        isQualityCritical: /\b(payment|결제|auth|인증|security|보안|production|운영|migration|마이그레이션|critical|중요|database|데이터베이스)/i.test(lower),
...
function buildTeamContext(task, rec, cwd) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        worktreeLines.push(`git worktree add -b "${branch}" "${wbase}/agent-${i + 1}" HEAD`);
        if (i === 0) {
            tmuxLines.push(`tmux new-session -d -s "${session}" -c "${wbase}/agent-1" ` +
                `'claude -p "${escaped}" --output-format json 2>&1 | tee /tmp/claude-spawn-${session}-1.json; echo "Agent 1 complete"; read'`);
        }
        else {
            tmuxLines.push(`tmux split-window -t "${session}" -c "${wbase}/agent-${i + 1}" ` +
    ...
    return [
        ...
        "Check outputs:",
        ...Array.from({ length: n }, (_, i) => `  cat /tmp/claude-spawn-${session}-${i + 1}.json`),
        "",
...
function buildRalphContext(task, rec) {
    const verifyCmd = rec.tasks[0]?.match(/--verify\s+"([^"]+)"/)?.[1] || null;
    return [
...
function buildAutopilotContext(task, rec) {
    return [
...
function buildPipelineContext(task, rec) {
    return [
...
function buildSwarmContext(task, rec) {
    return [
...
function buildQAContext(task, rec) {
    return [
...
function buildRalplanContext(task, rec) {
    return [
