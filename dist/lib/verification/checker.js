...
function extractBashExecutions(transcript) {
    const executions = [];
...
function checkExecution(executions, patterns, checkName) {
    const now = Date.now();
...
function checkTodos(cwd) {
    try {
        ...
        if (!changed)
            return { check: "TODO", status: "pass", evidence: "No changed files" };
...
function checkErrorFree(transcript) {
