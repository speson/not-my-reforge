...
export function loadHistory(cwd) {
    return readDataFile(cwd, FILENAME, { ...DEFAULT_HISTORY, records: [] });
}
export function saveHistory(cwd, history) {
...
export function getRecentRecords(cwd, count = 10) {
    const history = loadHistory(cwd);
    return history.records.slice(-count);
}
export function formatHistorySummary(records) {
    if (records.length === 0)
