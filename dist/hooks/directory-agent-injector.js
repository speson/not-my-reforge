...
function hasTsxOrJsxFiles(dirPath) {
    try {
...
function checkFrontendDir(cwd, dirName) {
    const full = join(cwd, dirName);
    if (!existsSync(full))
        return false;
    return hasTsxOrJsxFiles(full);
}
function getTopLevelEntries(cwd) {
    try {
...
function getRecentGitFiles(cwd) {
    try {
...
function recommendAgents(cwd) {
    const recommendations = [];
    const added = new Set();
    const entries = getTopLevelEntries(cwd);
    const entrySet = new Set(entries);
    function addIfNew(agent, reason) {
        if (!added.has(agent)) {
            added.add(agent);
    ...
    const hasRecentTests = recentFiles.some((f) => f.endsWith(".test.ts") || f.endsWith(".spec.ts"));
    if (hasRecentTests) {
