...
async function main() {
    ...
    const deliverables = readDataFile(cwd, "deliverables.json", null);
    if (!deliverables || deliverables.length === 0) {
        ...
        if (autoDeliverables.length === 0)
            process.exit(0);
        // Only verify string-based criteria that look like commands
        const commandCriteria = autoDeliverables.filter((d) => d.criteria.some((c) => c.value.includes(" ") && !c.value.includes("should") && !c.value.includes("must")));
        if (commandCriteria.length === 0)
            process.exit(0);
