...
type ErrorClass = "not_found" | "ambiguous" | "whitespace" | "stale" | "unknown";
...
function getGitDiffContext(filePath: string, cwd: string): string | null {
  if (!cwd) return null;
  try {
...
function getFileSnippet(filePath: string, oldString: string): string | null {
...
const RECOVERY_STRATEGIES: Record<ErrorClass, string[]> = {
  not_found: [
