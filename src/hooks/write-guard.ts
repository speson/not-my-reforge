...
function isRegeneratedFile(filePath: string): boolean {
  const name = basename(filePath);
  if (REGENERATED_FILENAMES.has(name)) return true;
  if (name.includes(".generated.") || name.endsWith(".g.dart")) return true;
...
async function main() {
  ...
  if (lineCount < 5) process.exit(0);

  // Allow regenerated files
  if (isRegeneratedFile(filePath)) process.exit(0);
