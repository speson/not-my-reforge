...
async function main() {
  ...
  if (teamState) {
    ...
    if (activeWorkers.length > 0) {
      suggestions.push(
        `${activeWorkers.length} teammate(s) still working: ${activeWorkers.map((w) => w.name).join(", ")}`
      );
    }
  }
  ...
  if (!isGit.ok) process.exit(0);

  const changedFiles = exec("git diff --name-only", cwd);
  if (!changedFiles.stdout && suggestions.length === 0) process.exit(0);
