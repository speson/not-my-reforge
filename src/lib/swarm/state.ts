...
import type { SwarmState, SwarmTask } from "./types.js";

const FILENAME = "swarm-state.json";

export function loadSwarmState(cwd: string): SwarmState | null {
  const state = readDataFile<SwarmState | null>(cwd, FILENAME, null);
  if (!state || !state.active) return null;
  return state;
}

export function saveSwarmState(cwd: string, state: SwarmState): void {
  state.lastActivityAt = new Date().toISOString();
  writeDataFile(cwd, FILENAME, state);
}

export function clearSwarmState(cwd: string): void {
  writeDataFile(cwd, FILENAME, { active: false });
}

export function getPendingTasks(state: SwarmState): SwarmTask[] {
  return state.tasks.filter((t) => t.status === "pending");
}

export function getAssignableTasks(state: SwarmState): SwarmTask[] {
...
export function assignTask(state: SwarmState, taskId: number): SwarmTask | null {
...
export function completeTask(state: SwarmState, taskId: number, result: string): void {
  const task = state.tasks.find((t) => t.id === taskId);
  if (!task) return;
  task.status = "done";
  task.result = result;
...
export function failTask(state: SwarmState, taskId: number, error: string): void {
  const task = state.tasks.find((t) => t.id === taskId);
  if (!task) return;
  task.status = "failed";
  task.error = error;
...
export function formatSwarmStatus(state: SwarmState): string {
  ...
  for (const t of state.tasks) {
    const icons: Record<string, string> = { pending: "-", assigned: ">", done: "v", failed: "x" };
    let line = `  [${icons[t.status]}] #${t.id} (${t.agentType}/${t.modelTier}): ${t.description}`;
    if (t.status === "failed" && t.error) line += ` — ${t.error}`;
    lines.push(line);
  }
...
export function formatSwarmContext(state: SwarmState): string {
...
export function formatSwarmReport(state: SwarmState): string {
  const lines = [
