...
export function generatePrMetadata(cwd: string, baseBranch?: string): PrMetadata | null {
...
export function detectCommitType(files: string[]): ConventionalType {
...
export function generateReleaseInfo(cwd: string, sinceTag?: string): ReleaseInfo {
  ...
  for (const c of commits) {
    ...
    else if (msg.includes("BREAKING") || msg.includes("!:")) breaking.push(msg);
    else other.push(msg);
  }
...
export function formatReleaseNotes(info: ReleaseInfo, newVersion: string): string {
  const lines = [`# Release ${newVersion}`, ""];

  if (info.breaking.length > 0) {
    lines.push("## Breaking Changes");
    for (const c of info.breaking) lines.push(`- ${c}`);
    lines.push("");
  }

  if (info.features.length > 0) {
    lines.push("## Features");
    for (const c of info.features) lines.push(`- ${c}`);
    lines.push("");
  }

  if (info.fixes.length > 0) {
    lines.push("## Bug Fixes");
    for (const c of info.fixes) lines.push(`- ${c}`);
    lines.push("");
  }

  if (info.other.length > 0) {
    lines.push("## Other Changes");
    for (const c of info.other.slice(0, 10)) lines.push(`- ${c}`);
    if (info.other.length > 10) lines.push(`- ... and ${info.other.length - 10} more`);
    lines.push("");
  }
