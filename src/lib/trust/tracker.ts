...
export function loadTrustState(cwd: string): TrustState {
  return readDataFile<TrustState>(cwd, TRUST_FILE, { ...DEFAULT_STATE });
}

function saveTrustState(cwd: string, state: TrustState): void {
 ...
 */
export function recordSuccess(cwd: string, toolName: string): TrustState {
 ...
 */
export function recordFailure(cwd: string): TrustState {
  const state = loadTrustState(cwd);
 ...
 */
export function recordBuildPass(cwd: string): TrustState {
  const state = loadTrustState(cwd);
 ...
 */
export function recordTestPass(cwd: string): TrustState {
  const state = loadTrustState(cwd);
 ...
 */
export function getTrustLevel(cwd: string): TrustLevel {
  return loadTrustState(cwd).level;
}

const LEVEL_NAMES: Record<TrustLevel, string> = {
  0: "strict",
  1: "familiar",
  2: "trusted",
  3: "autonomous",
 ...
 */
export function formatTrustStatus(cwd: string): string {
  const state = loadTrustState(cwd);
  ...
  if (level === 0) {
    lines.push(
      `Consecutive successes: ${consecutiveSuccesses}/${SUCCESSES_FOR_LEVEL_1} (next: ${SUCCESSES_FOR_LEVEL_1} successful Edit/Write ops)`,
    );
  } else if (level === 1) {
  ...
  lines.push(
    "",
    "Levels:",
    `  0 (strict)     — All operations need approval${level === 0 ? " [CURRENT]" : ""}`,
    `  1 (familiar)   — Edit auto-approved${level === 1 ? " [CURRENT]" : ""}`,
    `  2 (trusted)    — Write + build/test auto-approved${level === 2 ? " [CURRENT]" : ""}`,
    `  3 (autonomous) — Most operations auto-approved${level === 3 ? " [CURRENT]" : ""}`,
  );
