...
import type { TeamState, MergeResult, FileOverlap } from "./types.js";
...
export function detectFileOverlaps(cwd: string, state: TeamState): FileOverlap[] {
  const fileWorkerMap = new Map<string, number[]>();

  for (const worker of state.workers) {
    if (worker.status !== "done") continue;

    const diff = exec(
      `git diff --name-only "${state.baseBranch}...${worker.branch}"`,
      cwd
    );
    if (!diff.ok) continue;
...
export function getWorkerChangedFiles(
  cwd: string,
  baseBranch: string,
  workerBranch: string
): string[] {
...
export function checkMergeability(
  ...
  baseBranch: string,
  workerBranch: string
): { mergeable: boolean; conflictFiles: string[] } {
  // Dry-run merge to check for conflicts
  const result = exec(
    `git merge-tree $(git merge-base "${baseBranch}" "${workerBranch}") "${baseBranch}" "${workerBranch}"`,
    cwd
  );
  ...
  if (conflicts && conflicts.length > 0) {
    ...
    const conflictFiles = fileMatches.map((m) =>
...
export function suggestMergeOrder(cwd: string, state: TeamState): number[] {
