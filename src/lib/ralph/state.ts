...
export function loadRalphState(cwd: string): RalphState | null {
  const state = readDataFile<RalphState | null>(cwd, FILENAME, null);
  if (!state || !state.active) return null;
  return state;
}

export function saveRalphState(cwd: string, state: RalphState): void {
  writeDataFile(cwd, FILENAME, state);
}

export function clearRalphState(cwd: string): void {
  writeDataFile(cwd, FILENAME, { active: false });
}

export function advanceIteration(state: RalphState): void {
  state.iteration += 1;
  state.lastIterationAt = new Date().toISOString();
}

export function recordFailure(state: RalphState, reason: string): void {
  state.consecutiveFailures += 1;
  state.lastFailureReason = reason;
}

export function recordSuccess(state: RalphState): void {
  state.consecutiveFailures = 0;
  state.status = "success";
  state.active = false;
}

export function recordApproach(state: RalphState, approach: string): void {
...
export function isExhausted(state: RalphState): boolean {
  return state.iteration >= state.maxIterations;
}

export function getAdaptationStrategy(state: RalphState): string {
...
export function formatRalphContext(state: RalphState): string {
  const sections = [
    `[Ralph Loop Active]`,
    `Task: ${state.task}`,
