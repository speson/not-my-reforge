...
export interface IntentAnalysis {
  ...
  reasons: string[];
  suggestedConfirmation: string | null;
}

const DESTRUCTIVE_PATTERNS: Array<{ pattern: RegExp; risk: IntentAnalysis["riskLevel"]; reason: string }> = [
...
export function analyzeIntent(prompt: string): IntentAnalysis {
  ...
  let maxRisk: IntentAnalysis["riskLevel"] = "none";
  ...
  let suggestedConfirmation: string | null = null;
  if (maxRisk === "critical") {
...
export function formatIntentWarning(analysis: IntentAnalysis): string {
  if (!analysis.isDestructive) return "";
