...
const DEFAULT_STATE: QualityScoreState = {
  totalEdits: 0,
  successfulEdits: 0,
  buildPassed: null,
  testsPassed: null,
...
export function loadQualityScore(cwd: string): QualityScoreState {
  return readDataFile<QualityScoreState>(cwd, FILENAME, { ...DEFAULT_STATE });
}

function saveQualityScore(cwd: string, state: QualityScoreState): void {
  state.updatedAt = new Date().toISOString();
  writeDataFile(cwd, FILENAME, state);
}

export function recordEditResult(cwd: string, success: boolean): void {
...
export function recordBuildResult(cwd: string, passed: boolean): void {
  const state = loadQualityScore(cwd);
  state.buildPassed = passed;
  saveQualityScore(cwd, state);
}

export function recordTestResult(cwd: string, passed: boolean): void {
  const state = loadQualityScore(cwd);
  state.testsPassed = passed;
  saveQualityScore(cwd, state);
}

export function recordSlopComment(cwd: string): void {
  const state = loadQualityScore(cwd);
  state.slopComments += 1;
  saveQualityScore(cwd, state);
}

function computeComponents(state: QualityScoreState): ScoreComponents {
  ...
  const buildHealth =
    state.buildPassed === null ? 50 : state.buildPassed ? 100 : 0;

  // Test health: 100 pass, 0 fail, 50 not run
  const testHealth =
    state.testsPassed === null ? 50 : state.testsPassed ? 100 : 0;
  ...
  const overall = Math.round(
    editSuccessRate * WEIGHTS.editSuccessRate +
...
export function calculateScore(cwd: string): ScoreComponents {
  const state = loadQualityScore(cwd);
  return computeComponents(state);
}

export function formatScoreReport(cwd: string): string {
  const state = loadQualityScore(cwd);
