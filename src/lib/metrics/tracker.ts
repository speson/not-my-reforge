...
import type { SessionMetrics, ToolCallStat } from "./types.js";
import { EMPTY_METRICS } from "./types.js";

const FILENAME = "session-metrics.json";

export function loadMetrics(cwd: string): SessionMetrics {
  return readDataFile<SessionMetrics>(cwd, FILENAME, {
    ...EMPTY_METRICS,
...
export function saveMetrics(cwd: string, metrics: SessionMetrics): void {
...
export function recordAgentSpawn(metrics: SessionMetrics): void {
  metrics.agentsSpawned++;
  metrics.estimatedTokens += 2000; // Agent spawns use more tokens
}

export function getSessionDuration(metrics: SessionMetrics): string {
...
export function getTopTools(metrics: SessionMetrics, n = 5): ToolCallStat[] {
