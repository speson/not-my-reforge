...
async function main() {
    ...
    try {
        writeFileSync("/tmp/reforge-popup-done", "");
    }
    catch { }
    ...
    const outputStr = typeof input.tool_output === "object" && input.tool_output?.content
        ? input.tool_output.content
        : undefined;
    metrics.estimatedTokens += estimateOutputTokens(outputStr);
