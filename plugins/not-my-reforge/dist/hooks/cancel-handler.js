...
async function main() {
    ...
    if (target === "all" || target === "autopilot") {
        const autopilotState = readDataFile(cwd, "autopilot-state.json", null);
    ...
    if (target === "all" || target === "pipeline") {
        const pipelineState = readDataFile(cwd, "pipeline-state.json", null);
    ...
    if (target === "all" || target === "swarm") {
        const swarmState = readDataFile(cwd, "swarm-state.json", null);
    ...
    if (target === "all" || target === "qa") {
        const qaState = readDataFile(cwd, "qa-state.json", null);
        if (qaState?.active) {
    ...
    if (target === "all" || target === "ralplan") {
        const ralplanState = readDataFile(cwd, "ralplan-state.json", null);
