...
const SHORTCUTS: Shortcut[] = [
  ...
  // #verify — Run quality gate checks now
  {
    ...
    handler: (cwd) => {
      ...
      const metrics = readDataFile<{
        agents?: Array<{ agentType: string; durationMs?: number }>;
      } | null>(cwd, "agent-metrics.json", null);
  ...
  // #team — Team agent setup
  {
    name: "team",
    handler: (cwd, args) => {
      const teamState = readDataFile<{ active?: boolean; workers?: unknown[] } | null>(
        cwd, "team-state.json", null
      );
      ...
      if (teamState?.active) {
        lines.push(
          ...
          `Workers: ${teamState.workers?.length || 0}`,
          lockStatus,
          "",
          "Commands:",
          "  reforge cancel team  — Cancel team session",
        );
      } else {
        lines.push(
          ...
          "  reforge team 2 refactor the API layer and add tests",
          "",
          "Team composition: N workers + 1 reviewer",
  ...
  // #status — Session metrics
  {
    ...
    handler: (cwd) => {
      const modeStatus = formatModeStatus(cwd);
      const agentMetrics = readDataFile<{
        sessionTotals?: { totalSpawned: number; successCount: number; failureCount: number; totalDurationMs: number };
        byType?: Record<string, { totalCalls: number; avgDurationMs: number; successRate: number }>;
      } | null>(cwd, "agent-metrics.json", null);
      const lockStatus = formatOwnershipStatus(cwd);
      ...
      if (agentMetrics?.sessionTotals) {
        ...
        lines.push(
          "Agent Metrics:",
          `  Total spawned: ${t.totalSpawned}`,
          `  Success: ${t.successCount}, Failures: ${t.failureCount}`,
          `  Total agent time: ${Math.round(t.totalDurationMs / 1000)}s`,
        );
  ...
  // #memo — Notepad view/add
  {
    ...
    handler: (cwd, args) => {
      if (args) {
        // Add a note
        const priority = args.startsWith("!") ? "critical" as const : "normal" as const;
