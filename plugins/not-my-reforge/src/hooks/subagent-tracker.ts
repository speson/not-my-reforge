...
interface AgentMetrics {
  agents: AgentMetric[];
  byType: Record<string, AgentTypeStats>;
  sessionTotals: {
    totalSpawned: number;
    totalDurationMs: number;
    successCount: number;
...
function loadMetrics(cwd: string): AgentMetrics {
  return readDataFile<AgentMetrics>(cwd, FILENAME, {
    agents: [],
    byType: {},
    sessionTotals: {
      totalSpawned: 0,
...
function rebuildStats(metrics: AgentMetrics): void {
  const byType: Record<string, AgentTypeStats> = {};
...
async function main() {
  ...
  if (!cwd || input.tool_name !== "Task") process.exit(0);

  const metrics = loadMetrics(cwd);

  const subagentType = (input.tool_input?.subagent_type as string) || "unknown";
  const description = (input.tool_input?.description as string) || "";
  const model = input.tool_input?.model as string | undefined;
  const background = (input.tool_input?.run_in_background as boolean) || false;
