...
const MODE_TRIGGERS: { pattern: RegExp; mode: ModeName }[] = [
...
async function main() {
  ...
  if (/^reforge\s+(cancel|stop|abort)\b/i.test(prompt)) process.exit(0);
  ...
  if (isCancelCoolingDown(registry, trigger.mode)) {
    ...
    const remaining = Math.ceil(
      (sentinel.ttlMs - (Date.now() - sentinel.cancelledAt)) / 1000
    );
    writeOutput({
      hookSpecificOutput: {
